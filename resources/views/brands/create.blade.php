<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Add Brand') }}</h2>
    </x-slot>
            ...
            <div class="bg-white p-6 shadow-sm sm:rounded-lg border border-gray-100">
                <form action="{{ route('brands.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name')" required autofocus placeholder="e.g. Samsung" />
                    ...
                    <div class="mt-4">
                        ...
                        />
                        <p class="mt-1 text-xs text-gray-500">PNG, JPG, WEBP, GIF or SVG — max 5MB. Extra whitespace is cropped automatically so the logo fits cleanly in “Gadget Lovers” on the homepage. If you skip this, a simple text mark is used.</p>
                        <x-input-error class="mt-2" :messages="$errors->get('logo')" />
                    </div>

                    <div class="mt-4 flex items-center gap-2">
                        <input id="is_active" name="is_active" type="checkbox" value="1" checked class="rounded border-gray-300 text-indigo-600">
                    ...
                    <div class="mt-6 flex items-center justify-end gap-4">
                        ...
                        <x-primary-button>{{ __('Save Brand') }}</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
